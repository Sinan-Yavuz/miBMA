...
#' Specialized bicreg function
#'
#' This function returns some necessary information such as residuals
#' @param x.
#' @param y.
#'@author this is a different version of bicreg which was written
#'by Adrian Raftery, Jennifer Hoeting, Chris Volinsky, Ian Painter, Ka Yee Yeung, 2018
#' @export

my.bicreg <-
  function (x, y, wt = rep(1, length(y)), strict = FALSE, OR = 20,
            maxCol = 31, drop.factor.levels = TRUE, nbest = 150, user.int = TRUE)
  {
    ...
    nvar <- length(x[1, ])
    if (nvar > 2) {
      a <- regsubsets(x, y, weights = wt, nbest = nbest, nvmax = ncol(x),
    ...
    result <- list(postprob = postprob, namesx = xnames, label = label,
                   ...
                   n.models = length(postprob), n.vars = length(probne0), fit = fit, residuals = residuals, coef = coeff)
    class(result) <- "bicreg"
    if (user.int) {
    }
